...
3. **全协议科学上网 (全内置 ShellCrash + Mihomo + Yacd)**：
   ...
   - **图形化控制台**：内置 Yacd Web 控制面板 (`http://192.168.123.1:9999/ui`)，图形化一键测速、分流与节点切换。
...
3. **默认管理信息**：
   ...
   > ⚠️ 如果您已在 Breed 中修改过 LAN IP（如改为 `192.168.2.1`），请将上述地址替换为您的实际 IP。
...
3. **可视化节点测速与选择**：
   - 点击页面上的 **「在新窗口打开 Web 控制面板」**，或者直接访问 `http://192.168.123.1:9999/ui`；
